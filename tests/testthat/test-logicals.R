...
test_that("logical operator error checking works", {
  ...
  expect_error(1 %c% 1)
  expect_error(1 %c% "")
  expect_error(1 %c% "| > 0", "not to start or end")
  expect_error(1 %c% "> 0 |", "not to start or end")
  expect_error(1 %c% "& > 0", "not to start or end")
  expect_error(1 %c% "> 0 &", "not to start or end")
  expect_error(1 %c% "> 0 || < 2", "adjacent")
  ...
  expect_error(0 %e% "(15, 1) & (15, 1)")
  expect_error(0 %e% 1, "character string")
  expect_error(0 %e% c("(0, 1)", "(2, 3)"), "exactly one")
  expect_error(0 %e% "", "non-empty")
  expect_error(0 %e% "| (0, 1)", "not to start or end")
  expect_error(0 %e% "(0, 1) |", "not to start or end")
  expect_error(0 %e% "& (0, 1)", "not to start or end")
  expect_error(0 %e% "(0, 1) &", "not to start or end")
  expect_error(0 %e% "(0, 1) || (2, 3)", "adjacent")
